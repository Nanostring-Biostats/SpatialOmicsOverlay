...
```{r Load Libraries}
library(SpatialOmicsOverlay, quietly = TRUE)
library(GeomxTools, quietly = TRUE)
```
...
There are 2 options to add a legend of the visualization markers used. 

The first is an easy way for data exploration, adding a legend to the ggplot object. 
...
The second requires more user manipulation but creates a more publication-ready figure. 
...
legend <- fluorLegend(muBrain, nrow = 2, textSize = 4, 
                      boxColor = "grey", alpha = 0.3)

cowplot::ggdraw() +
    cowplot::draw_plot(gp) +
    cowplot::draw_plot(legend, scale = 0.1, x = 0.1, y = -0.25)
```
